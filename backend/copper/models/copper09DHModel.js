...
const copper09DHSchema = new Schema({
    ...
    rng3odAverage: {type: Number, default: function() {
        return (Math.round((this.rng3od1 + this.rng3od2) / 2))
    }},
    rng4od1: {type: Number, required: true},
    rng4od2: {type: Number, required: true},
    rng4odAverage: {type: Number, default: function() {
        return (Math.round((this.rng4od1 + this.rng4od2) / 2))
    }}
}, { timestamps: true })

module.exports = mongoose.model('Copper09DH', copper09DHSchema)