...
const Copper09DHForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < selectedNumber; i++) {
      const copper09DH = inputData[i];

      const response = await fetch("/copper09DH", {
        method: "POST",
        body: JSON.stringify(copper09DH),
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch({ type: "CREATE_COPPER09DH", payload: json });
      }
    }
  };

  const inputRows = inputData.map((input, index) => (
    <div className="cu09DHFormGrid" key={index}>
      <input
        type="text"
