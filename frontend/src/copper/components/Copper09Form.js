...
const Copper09Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < selectedNumber; i++) {
      const copper09 = inputData[i];

      const response = await fetch("/copper09", {
        method: "POST",
        body: JSON.stringify(copper09),
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch({ type: "CREATE_COPPER09", payload: json });
      }
    }
  };
