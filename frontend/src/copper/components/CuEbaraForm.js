...
const CuEbaraForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < selectedNumber; i++) {
      const cuEbara = inputData[i];

      const response = await fetch('/cuEbara', {
        method: 'POST',
        body: JSON.stringify(cuEbara),
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch({ type: 'CREATE_CUEbara', payload: json });
      }
    }
  ...
  return (
    <form className="create" onSubmit={handleSubmit}>
      <div>
        <h2 className="formHeader">Cu Ebara</h2>
